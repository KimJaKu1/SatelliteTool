...
import org.sat_tool.domain.event.service.AntennaTrackingService;
import org.sat_tool.domain.event.service.ContactScheduleService;
import org.sat_tool.domain.event.service.EclipseService;
...
public class GenerateEvent {
    ...
    void generateATFile() {
        ...
        sat.setTle(new TLE(line1, line2));
        sat.setOrbitNumber((long) sat.getTle().getRevolutionNumberAtEpoch());

        List<Station> stations = new ArrayList<>();
        stations.add(new Station("TestStn", 36.8663, 127.1530, 100.0, List.of(0)));

        atService.generateAntennaTracking(
                sat,
                stations,
                timeConverter.localDateTimeUtcToAbsoluteDate(startTime),
                timeConverter.localDateTimeUtcToAbsoluteDate(endTime),
                60.0).thenCompose(map -> atService.generateATFile(map.entrySet(), Path.of(path))).join();
    }
    ...
    void generateNCFile() {
        ...
        var nc = ncService.computeNodalCrossing(sat,
                startAbsoluteDate, endAbsoluteDate, 60.0, true);
    ...
    void generateCSFile() {
        ...
        csService.generateContactSchedule(
                ...
                timeConverter.localDateTimeUtcToAbsoluteDate(startTime),
                timeConverter.localDateTimeUtcToAbsoluteDate(endTime),
                60.0).thenCompose(map -> csService.generateCSFile(map.entrySet(), Path.of(path))).join();
    }
