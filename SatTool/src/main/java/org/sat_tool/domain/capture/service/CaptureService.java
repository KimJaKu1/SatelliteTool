package org.sat_tool.domain.capture.service;
...
import org.sat_tool.domain.capture.writer.CaptureReportWriter;
...
public class CaptureService {
    ...
    private final CaptureReportWriter reportWriter;

    public CaptureService(PropagatorService propagatorService, CaptureReportWriter reportWriter) {
    ...
    public List<ImagingOpportunity> computeScheduleWithFootprintsFromEcef_NoPropagator(
        ...
        CaptureStateProvider provider = date -> {
            PVCoordinates pv = HermiteEventUtils.pvAtTimeNoProp(
                    date,
                    series.timesUtc(),
    ...
    public void generateCaptureFile(List<ImagingOpportunity> opportunities, String satName, Path outputDir) {
        reportWriter.writeCaptureFile(opportunities, satName, outputDir);
    }
    ...
    private ImagingOpportunity buildOpportunity(
        ...
        List<LLA> footprint = footprintAtInstant(
                earth,
                itrs,
                captureTime,
    ...
    private List<LLA> footprintAtInstant(
            OneAxisEllipsoid earth,
            Frame itrs,
            AbsoluteDate date,
            Vector3D satEcef,
        ...
        List<LLA> polygon = new ArrayList<>(4);
        ...
        for (int[] corner : corners) {
            ...
            polygon.add(LLA.ofWgs84Degrees(
    ...
    private FovGeometry buildFovGeometry(FovParams fov) {
        double widthMeters = fov.getWpx() * fov.getPixelPitchM();
        double heightMeters = fov.getHpx() * fov.getPixelPitchM();
        return new FovGeometry(
                Math.atan((widthMeters / 2.0) / fov.getFocalLengthM()),
                Math.atan((heightMeters / 2.0) / fov.getFocalLengthM())
        );
    }
